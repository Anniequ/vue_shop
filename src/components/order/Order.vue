<template>
  <div>
    ...
    <el-dialog
      ...
      width="50%"
      @close="proDialogClosed"
    >
      <el-timeline>
        <el-timeline-item
          v-for="(proInfo, index) in proInfo"
          :key="index"
          :timestamp="proInfo.time"
        >
          {{ proInfo.context }}
        </el-timeline-item>
      </el-timeline>
...
export default {
  ...
  methods: {
    ...
    async showProgress() {
      ...
      // const { data: res } = await this.$http.get(`/kuaidi/804909574412544580`) //不能用
      if (res.meta.status !== 200)
